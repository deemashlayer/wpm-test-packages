...
<META HTTP-EQUIV="Expires" CONTENT="-1">
		...
		if(!isValidRedirectURL(redirectURL)){
          ...
          return;
		   ...
		   var url = authurl.concat("?response_type=code&client_id=").concat(clientID).concat("&scope=").concat(scope).concat("&redirect_uri=").concat(redirectURL);
				...
				if(( authCodeValueRecived == null ) || ( authCodeValueRecived == "" ) || isblank(authCodeValueRecived)){
					windowOpen('false');
				}
				}, 2000);   
	  }
	...
	if (event.origin !== redirectURL){
		...
		var containsCode = currentURL.includes("code=");
		if(containsCode){
			authCodeValueRecived = "true";
			...
			alert("Received Authorization Code");
